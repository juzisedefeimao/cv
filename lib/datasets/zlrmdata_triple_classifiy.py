...
class zlrmdata_triple_classify( ):
    def __init__(self, image_set, devkit_path=None):
        self.name = 'zlrmdata_triple_classify'
        self._image_set = image_set
        self._devkit_path = self._get_default_path() if devkit_path is None \
                            else devkit_path
        self._data_path = os.path.join(self._devkit_path, 'zlrmdata_triple_classify')
        self._classes = ('defect_tree_0_0', 'defect_tree_0_1')  # 背景，纹理，夹渣，铝屑，裂纹，边裂，油污
    ...
    def cache_path(self):
        cache_path = osp.abspath(osp.join(cfg.ZLRM.DATA_DIR, 'zlrmdata_triple_classify', 'datasets', 'cache'))
        ...
        for name in db_inds:
            if name == 'unlabel':
                for i in db_inds[name]:
                    minibatch_db['unlabel'].append(self.imagedb[name][i])
            else:
                for i in db_inds[name]:
                    minibatch_db['label'].append(self.imagedb[name][i])
    ...
    def get_epoch_imagedb(self):
        ...
        for name in self.imagedb:
            if name == 'unlabel':
                epoch_imagedb['unlabel'] = epoch_imagedb['unlabel'] + self.imagedb[name]
            else:
                for i in range(len(self.imagedb[name])):
                    if i < (cfg.GAN.TRAIN.LABEL_DATA_NUM // self.num_classes()):
                        epoch_imagedb['label'].append(self.imagedb[name][i])
                        epoch_imagedb['unlabel'].append(self.imagedb[name][i])
                    epoch_imagedb['unlabel'].append(self.imagedb[name][i])
