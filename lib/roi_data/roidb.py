...
# <<<< obsolete
from lib.rpn_msr.bbox import bbox_overlaps
...
def add_bbox_regression_targets(roidb):
    ...
    if cfg.ZLRM.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
        ...
        means = np.tile(
                np.array(cfg.ZLRM.TRAIN.BBOX_NORMALIZE_MEANS), (num_classes, 1))
        stds = np.tile(
                np.array(cfg.ZLRM.TRAIN.BBOX_NORMALIZE_STDS), (num_classes, 1))
    else:
        # Compute values needed for means and stds
        # var(x) = E(x^2) - E(x)^2
        class_counts = np.zeros((num_classes, 1)) + cfg.ZLRM.EPS
    ...
    # Normalize targets
    if cfg.ZLRM.TRAIN.BBOX_NORMALIZE_TARGETS:
...
def _compute_targets(rois, overlaps, labels):
    ...
    ex_inds = np.where(overlaps >= cfg.ZLRM.TRAIN.BBOX_THRESH)[0]
