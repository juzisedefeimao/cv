...
batch = cfg.SIAMSE.TRAIN.BATCH_SIZE * cfg.SIAMSE.IMAGE_TRANSFORM_NUM
class Siammask_train(Network):
    def __init__(self):
        self.inputs = []
        self.anchor_num = 9
        ...
        self.gt_boxes = tf.placeholder(tf.float32, shape=[batch*classes_num, None, 5], name='gt_boxes')
        self.layers = {'search_data': self.search_data, 'template_data': self.template_data,
                       'im_info': self.im_info, 'gt_boxes': self.gt_boxes}
        self.setup()

    def build_loss(self):

        rpn_cls_score_reshape, rpn_data = \
            self.roi_proposal(self.layers['feature_sum_cls'], self.layers['feature_sum_loc'])

        rpn_cls_score = tf.reshape(rpn_cls_score_reshape, [-1, 2])  # shape (batch*classes*HxWxA, 2)
        rpn_label = tf.reshape(rpn_data[0], [-1])  # shape (batch*classes*HxWxA)
        # ignore_label(-1)
        ...
        # box loss
        rpn_bbox_pred = self.layers['feature_sum_loc']  # shape (batch*classes, H, W, Ax4)
        ...
        rpn_loss_box_n = tf.reduce_sum(self.smooth_l1_dist(
            rpn_bbox_inside_weights * (rpn_bbox_pred - rpn_bbox_targets), sigma2=1), axis=[1])
        rpn_loss_box = tf.reduce_sum(rpn_loss_box_n) / (tf.reduce_sum(tf.cast(fg_keep, tf.float32)) + 1.0)
        # rpn_loss_box = tf.reduce_sum(rpn_loss_box_n) / 48
    ...
    def resnet50(self, data, reuse=False, bn_trainable = True):
    ...
    # =========================RPN============================================
    def rpn(self, search, template, anchor_num, name, reuse=False, bn_trainable=True):
    ...
    def roi_proposal(self, feature_sum_cls, feature_sum_loc):
        ...
        with tf.variable_scope('roi_proposal') as scope:
            ...
            # )
            (
                self.feed(feature_sum_cls, 'gt_boxes', 'im_info')
                    .siammase_anchor_target_layer(cfg.SIAMSE.FEAT_STRIDE, name='rpn_data')
            )

            return self.get_output('rpn_cls_score_reshape'), self.get_output('rpn_data')