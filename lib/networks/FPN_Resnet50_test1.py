...
class FPN_Resnet50_test(Network):
    ...
    def setup(self):
        ...
        with tf.variable_scope('newconv') as scope:
            ...
            # 将roi-data数据按p2,p3,p4,p5,p6分割为roi-data/p2,roi-fata/p3...
            (
                self.feed('rpn_rois')
                    .roi_segment(name='roi_segment')
            )
            # p2
            (
            ...
            (
                self.feed('cls_prob/P2')
                    .reshape_layer([-1, cfg.ZLRM.N_CLASSES + 1], name='cls_prob_reshape/P2')
            )
            (
                self.feed('cls_prob/P3')
                    .reshape_layer([-1, cfg.ZLRM.N_CLASSES + 1], name='cls_prob_reshape/P3')
            )
            (
                self.feed('cls_prob/P4')
                    .reshape_layer([-1, cfg.ZLRM.N_CLASSES + 1], name='cls_prob_reshape/P4')
            )
            (
                self.feed('cls_prob/P5')
                    .reshape_layer([-1, cfg.ZLRM.N_CLASSES + 1], name='cls_prob_reshape/P5')
            )
            (
                self.feed('cls_prob/P6')
                    .reshape_layer([-1, cfg.ZLRM.N_CLASSES + 1], name='cls_prob_reshape/P6')
            )
            (
                self.feed('cls_prob_reshape/P2',
                          'cls_prob_reshape/P3',
                          'cls_prob_reshape/P4',
                          'cls_prob_reshape/P5',
                          'cls_prob_reshape/P6')
                    .concat(0, name='cls_prob')
            )
            (
            ...
            (
                self.feed('ave_bbox_pred_rois_reshape/P2',
                    ...
                    .concat(0, name='ave_bbox_pred_rois')
            )
