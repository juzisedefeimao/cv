...
class Resnet50_test_rpn(Network):
    def __init__(self):
        self.inputs = []
    ...
    def setup(self):
        ...
        (
            self.feed('res3_6_relu')
                ...
                .conv(1, 1, len(cfg.ZLRM.ANCHOR_SCALE) * len(cfg.ZLRM.ANCHOR_RATIO) * 2, 1, 1, name='rpn_cls_score',
                      relu=False, padding='VALID', trainable=True)
        )
        (
            self.feed('rpn_conv')
                .conv(1, 1, len(cfg.ZLRM.ANCHOR_SCALE) * len(cfg.ZLRM.ANCHOR_RATIO) * 4, 1, 1, name='rpn_bbox_pred',
                      relu=False, padding='VALID', trainable=True)
        )
        # (
        ...
        (
            self.feed('rpn_cls_prob')
                .spatial_reshape_layer(len(cfg.ZLRM.ANCHOR_SCALE) * len(cfg.ZLRM.ANCHOR_RATIO) * 2,
