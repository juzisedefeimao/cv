...
class Resnet50_train_ohem(Network):
    def __init__(self):
        self.inputs = []
    ...
    def setup(self):
        ...
        (
            self.feed('transform3_bn', 'res3_6_bn3')
                ...
                .conv(1, 1, 2048, 1, 1, name='transform4_conv', relu=False, trainable=True)
                .batch_normalization(name='transform4_bn', relu=False, trainable=False)
        )
        # ======================第四组模块===========================
        (
            self.feed('res3_6_relu')
                .conv(1, 1, 512, 1, 1, name='res4_1_conv1', relu=False, trainable=True)
        ...
        (
            self.feed('res4_3_relu')
                .conv(1, 1, 1024, 1, 1, name='conv_new_1', relu=True, trainable=True)
                .conv(1, 1, (cfg.ZLRM.N_CLASSES + 1) * cfg.ZLRM.PSROIPOOL * cfg.ZLRM.PSROIPOOL, 1, 1, name='rfcn_cls', trainable=True)
        )
        (
            self.feed('conv_new_1')
                .conv(1, 1, 4 * (cfg.ZLRM.N_CLASSES + 1) * cfg.ZLRM.PSROIPOOL * cfg.ZLRM.PSROIPOOL, 1, 1, name='rfcn_bbox', trainable=True)
        )
