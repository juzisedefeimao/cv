...
def proposal_target_layer(rpn_rois, gt_boxes, _num_classes):
    ...
    fg_rois_per_image = int(np.round(cfg.ZLRM.TRAIN.FG_FRACTION * rois_per_image))
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    fg_inds = np.where(max_overlaps >= cfg.ZLRM.TRAIN.FG_THRESH)[0]
    ...
    bg_inds = np.where((max_overlaps < cfg.ZLRM.TRAIN.BG_THRESH_HI) &
                       (max_overlaps >= cfg.ZLRM.TRAIN.BG_THRESH_LO))[0]
...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
    ...
    by the networks (i.e. only one class has non-zero targets).
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    if cfg.ZLRM.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
        # Optionally normalize targets by a precomputed mean and stdev
        targets = ((targets - np.array(cfg.ZLRM.TRAIN.BBOX_NORMALIZE_MEANS))
                / np.array(cfg.ZLRM.TRAIN.BBOX_NORMALIZE_STDS))
    return np.hstack(
