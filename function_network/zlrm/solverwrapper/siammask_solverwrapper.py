...
from lib.roi_data.layer import RoIDataLayer
from lib.utils.timer import Timer
from lib.gt_data_layer import roidb as gdl_roidb
from lib.roi_data import roidb as rdl_roidb
...
class SolverWrapper(object):
    ...
    def snapshot(self, sess, iter):
        ...
        infix = ('_' + cfg.SIAMSE.TRAIN.SNAPSHOT_INFIX
                 if cfg.SIAMSE.TRAIN.SNAPSHOT_INFIX != '' else '')
        filename = (cfg.SIAMSE.TRAIN.SNAPSHOT_PREFIX + infix +
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        # optimizer
        if cfg.SIAMSE.TRAIN.SOLVER == 'Adam':
            opt = tf.train.AdamOptimizer(cfg.SIAMSE.TRAIN.LEARNING_RATE)
        elif cfg.SIAMSE.TRAIN.SOLVER == 'RMS':
            opt = tf.train.RMSPropOptimizer(cfg.SIAMSE.TRAIN.LEARNING_RATE)
        else:
            lr = tf.Variable(cfg.SIAMSE.TRAIN.LEARNING_RATE, trainable=False)
            # lr = tf.Variable(0.0, trainable=False)
            momentum = cfg.SIAMSE.TRAIN.MOMENTUM
        ...
        for iter in range(restore_iter, max_iters):
            ...
            # learning rate
            if iter != 0 and iter % cfg.SIAMSE.TRAIN.STEPSIZE == 0:
                sess.run(tf.assign(lr, lr.eval() * cfg.SIAMSE.TRAIN.GAMMA))

            # get one batch
            blobs = data_layer.forward()
            template = self.imdb.template()

            if (iter + 1) % (cfg.ZLRM.TRAIN.DISPLAY) == 0:
                print ('image: %s' %(blobs['im_name']),)

            feed_dict={
                self.net.search_data: blobs['data'],
                self.net.template_data: template,
            ...
            if (iter) % (cfg.ZLRM.TRAIN.DISPLAY) == 0:
                print(
                    'iter: %d / %d, total loss: %.4f, rpn_loss_cls: %.4f, rpn_loss_box: %.4f, lr: %f' % \
