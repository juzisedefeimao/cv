...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_epoch, restore=False):
        ...
        for epoch in range(restore_epoch, max_epoch):
            ...
            for iter in range(train_data_layer.epoch_iter_num):
                timer.tic()
                ...
                # get one batch
                blobs_train = train_data_layer.epoch_forward()
                ...
                if (iter) % (cfg.ZLRM.TRAIN.DISPLAY) == 0:
                    print(
                        'epoch: %d / %d, iter: %d / %d, loss: %.4f, test_loss: %.4f, lr: %f' % \
                        (epoch, max_epoch, iter, train_data_layer.epoch_iter_num, loss_train_val, loss_test_val, lr.eval()))
                    print ('speed: {:.3f}s / iter'.format(_diff_time))

            if (epoch+1) % cfg.ZLRM.TRAIN.SNAPSHOT_EPOCH == 0:
...
def train_net(network, imdb_train, imdb_test, output_dir, pretrained_model=None, max_epoch=40000, restore=False):
